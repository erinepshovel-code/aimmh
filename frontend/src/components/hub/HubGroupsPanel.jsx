    ...
    <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
      ...
      <form onSubmit={submit} className="mt-4 space-y-3 rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4">
        <div className="flex items-center justify-between gap-2">
          ...
          {editingId && <button type="button" onClick={() => { setEditingId(''); setForm(emptyForm); }} className="text-xs text-zinc-500 hover:text-zinc-300">Cancel edit</button>}
        </div>
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-3">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {selectableMembers.length === 0 ? <div className="text-xs text-zinc-500">Create instances first.</div> : selectableMembers.map((option) => {
              ...
              return (
                <label key={buildMemberKey(option)} className="flex items-start gap-2 rounded-xl border border-zinc-800 bg-zinc-950/70 px-3 py-2 text-xs text-zinc-300">
                  <input type="checkbox" checked={checked} onChange={() => toggleMember(option)} className="mt-0.5" />
      ...
      <div className="mt-4 space-y-3">
        {groups.length === 0 ? (
          ...
          <article key={group.group_id} className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                <button onClick={() => startEdit(group)} className="rounded-xl border border-zinc-800 px-3 py-2 text-xs text-zinc-300 transition hover:border-zinc-700 hover:text-white">
                  <span className="flex items-center gap-2"><Edit3 size={13} /> Edit</span>
                </button>
                <button onClick={() => onToggleArchive(group)} className="rounded-xl border border-zinc-800 px-3 py-2 text-xs text-zinc-300 transition hover:border-zinc-700 hover:text-white">
