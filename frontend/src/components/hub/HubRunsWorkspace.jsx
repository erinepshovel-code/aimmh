      ...
      <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <label className="flex items-center gap-2 text-xs text-zinc-400">
            <input type="checkbox" checked={includeArchivedRuns} onChange={(event) => setIncludeArchivedRuns(event.target.checked)} /> Show archived
          </label>
        </div>
        <div className="mt-4 space-y-3">
          {runs.length === 0 ? (
            ...
            <article key={run.run_id} className={`w-full rounded-2xl border p-4 transition ${selectedRunId === run.run_id ? 'border-emerald-500/40 bg-emerald-500/10' : 'border-zinc-800 bg-zinc-950/60 hover:border-zinc-700'}`}>
              <button onClick={() => setSelectedRunId(run.run_id)} className="w-full text-left">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button onClick={() => onToggleRunArchive(run)} className="rounded-xl border border-zinc-800 px-3 py-2 text-xs text-zinc-300 hover:border-zinc-700 hover:text-white">
                  <span className="flex items-center gap-2">{run.archived ? <RotateCcw size={13} /> : <Archive size={13} />} {run.archived ? 'Restore' : 'Archive'}</span>
                </button>
                {run.archived && (
                  <button onClick={() => onDeleteArchivedRun(run.run_id)} className="rounded-xl border border-zinc-800 px-3 py-2 text-xs text-zinc-300 hover:border-red-500/30 hover:text-red-300">
