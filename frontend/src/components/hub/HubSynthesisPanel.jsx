...
export function HubSynthesisPanel({
  ...
  return (
    <div className="space-y-4" data-testid="hub-synthesis-panel">
      ...
      >
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-zinc-800 bg-zinc-950/70 px-3 py-2 text-xs text-zinc-300" data-testid="synthesis-queue-persistence-row">
          <span>
            Queue persistence: {queuePersistenceScope === 'cloud' ? 'Cloud (paid account)' : queuePersistenceScope === 'local' ? 'Local browser (logged-in free)' : 'Session only (guest)'}
          </span>
          <label className="flex items-center gap-2" data-testid="synthesis-queue-persistence-toggle-label">
            <input
              type="checkbox"
              checked={persistSynthesisQueue}
              onChange={(event) => setPersistSynthesisQueue(event.target.checked)}
              disabled={queuePersistenceScope === 'session'}
              data-testid="synthesis-queue-persistence-toggle"
            />
            Persist queue
          </label>
        </div>
              ...
              <div className="grid gap-3 md:grid-cols-[280px_1fr]">
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-3">
                  ...
                  <button
                    ...
                    className="mt-3 rounded-xl border border-zinc-800 px-2 py-1 text-xs text-zinc-300 hover:text-white"
                  ...
                  <div key={output.message_id || `${batch.synthesis_batch_id}-${output.synthesis_instance_id}`} className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-3" data-testid={`synthesis-history-output-${output.message_id || `${batch.synthesis_batch_id}-${output.synthesis_instance_id}`}`}>
                    <div className="grid gap-3 md:grid-cols-[260px_1fr]">
                      <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-3 text-xs text-zinc-400">
                        <div>{output.synthesis_instance_name}</div>
                        <div className="mt-1">{output.model}</div>
                      </div>
                      <div><ResponseMarkdown content={output.content} fontScale={1} /></div>
                    </div>
                  </div>
                ))}
              </div>
